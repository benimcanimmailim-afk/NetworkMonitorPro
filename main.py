...
class Api:
    ...
    def start_port_scan(self, ip):
        def run():
            ...
            res_str = "\n".join([f"Port {p}: {'AÇIK' if s else 'KAPALI'}" for p, s in results])
            if self.window:
                msg = json.dumps(f"{ip} için Port Tarama Sonuçları:\n{res_str}")
        ...
        def run():
            ...
            if os.name == 'nt':
                ...
                if shutil.which("traceroute"):
                    ...
                    if self.window:
                        msg = json.dumps("Hata: 'traceroute' komutu bulunamadı.")
                        self.window.evaluate_js(f"alert({msg})")
                    return
                kwargs = {}
            ...
            try:
                ...
                if self.window:
                    msg = json.dumps(f"{ip} için Rota İzleme (Traceroute) Sonuçları:\n{res.stdout}")
                    self.window.evaluate_js(f"alert({msg})")
            except Exception as e:
                if self.window:
                    msg = json.dumps(f"Hata oluştu: {str(e)}")
                    self.window.evaluate_js(f"alert({msg})")
        threading.Thread(target=run, daemon=True).start()
        return True
...
if __name__ == "__main__":
    ...
    api = Api()
    window = webview.create_window('Ultra Network Monitor V5 - Premium NOC Konsolu', 'ui/index.html', js_api=api, width=1280, height=800)
